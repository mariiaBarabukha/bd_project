...
namespace GUI.ViewModels.MainMenu.AddEditPanels.ProductsInMarketOptions
{
    class AddViewModel
    {
        ...
        Action _back;
        public AddViewModel(Action back)
        {
            _back = back;
            ...
            var temp = Model.getInstance().db.GetProductsNotInMarket();
        ...
        private void Add()
        {
            try
            {
                Model.getInstance().db.AddNotPromProductInMarket(_dBProducts[Names.IndexOf(Product)].ID, Price, Amount);
            }
            catch (Exception e)
            {
